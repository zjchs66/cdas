...
import com.xjj.flink.function.cdas.table.CommonOperation;
 ...
 * concrete implementation of a {@link Elasticsearch7ApiCallBridge}, which is provided to the
 ...
 */
@Internal
public abstract class ElasticsearchSinkBase<C extends AutoCloseable> extends RichSinkFunction<RowData>
...
//    private final ElasticsearchSinkFunction<T> elasticsearchSinkFunction;
    ...
    /**
     * Provided to the user via the {@link} to add {@link ActionRequest
     * ActionRequests}.
     */
...
//                        + "The object probably contains or references non-serializable fields.");
    ...
    public void invoke(RowData value, Context context) throws Exception {
        ...
        Operator operator = deserialize(value);
        process(operator, requestIndexer);
    }


    public void process(Operator element, RequestIndexer indexer) {
        switch (element.getOpType()) {
            case "c":
            case "r":
            case "u":
            ...
            case "d":
                ...
                break;
            default:
                throw new TableException("Unsupported message kind: " + element.getOpType());
        }
    }

    private void processUpsert(Operator row, RequestIndexer indexer) {
        final String document = serialization(row);
        final String key = extractKey(row.getPr());
        if (key != null) {
            final UpdateRequest updateRequest =
                    new UpdateRequest(row.getTablename(), key)
                            .doc(document, XContentType.JSON)
                            .upsert(document, XContentType.JSON);
            indexer.add(updateRequest);
        } else {
            final IndexRequest indexRequest =
                    new IndexRequest(row.getTablename()).id(key).source(document, XContentType.JSON);
            ;
            indexer.add(indexRequest);
        }
    }

    private void processDelete(Operator row, RequestIndexer indexer) {
        final String key = extractKey(row.getPr());
        final DeleteRequest deleteRequest = new DeleteRequest(row.getTablename(), key);
        indexer.add(deleteRequest);
    }

    String serialization(Operator row) {
        Map<String, Object> document = new HashMap<>();
        for (Map.Entry<String, Object> entry : row.getColumnsValue().entrySet()) {
            document.put(entry.getKey(), entry.getValue());
        }
        document.put("dbz_op_type", row.getOpType());
        document.put("dbz_op_time", row.getOpts());
        try {
    ...
    String extractKey(Map<String, Object> prs) {
        if (prs == null || prs.size() < 1) {
            return null;
        }
        String key = "";
        int j = 0;
        for (Map.Entry<String, Object> entry : prs.entrySet()) {
            if (j > 0) {
                key += "_";
            }
            key += entry.getValue();
